...
const MemberCard = ({
  ...
  return (
    <div
      className={
        styles["member-card"] + (item.selected ? " " + styles["selected"] : "")
      }
    ...
    >
      <Row>
        <Col span={24}>
          ...
          <h2 className={styles["card-title"]}>{item?.memberType}</h2>
          <Row gutter={12}>
...
const Talents = () => {
  ...
  const dataList = [
    {
      ...
      base: "./vn.svg",
      avatar: "./fe.svg",
      tags: ["E-commerce", "SaaS", "Blockchain"],
      level: "Senior",
  ...
  const [selectedNumber, setSelectedNumber] = useState(
    data.reduce((count, item) => count + (item.selected ? 1 : 0), 0)
  );
  ...
  const handleSelect = (member, index) => {
    ...
    setSelectedNumber(
      list.reduce((count, item) => count + (item.selected ? 1 : 0), 0)
    );
    setData(list);
  };
  ...
  useEffect(() => {
    ...
    setSelectedIndustry(
      localStorage.getItem("industry") || industryOptions[0].value
    );
    setSelectedMember(
      localStorage.getItem("member") || membersOptions[0].value
    );
  }, []);
