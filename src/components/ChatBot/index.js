...
const Chatbot = () => {
  ...
  React.useEffect(() => {
    const conversationData = [
      ...
      {
        sender: "user",
        content: "I want to create restaurant reservation website!",
      },
      ...
      <div className={styles["chat-window"]}>
        {messages.map((message, index) => (
          <div
            key={index}
            className={
              styles[message.sender === "bot" ? "bot-message" : "user-message"]
            }
          >
            {message.content !== "Click here to build your product!" ? (
              ...
              <p>
                {" "}
                <>
                  <span onClick={showModal}>Click here</span> to see our feature
                  suggestions!
                </>
              </p>
            )}
